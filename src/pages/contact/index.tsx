...
import HeadContact from "./headingcontact"
  ...
  return (
    <div className="flex flex-col gap-2 w-full lg:max-w-[363px]">
      ...
      <input
        ...
        type={type}
        placeholder={label}
        className="bg-transparent border border-black rounded-md text-sm sm:text-base text-white placeholder:text-white py-3.5 px-4 focus:outline-0 focus:border-blue"
        required
...
const ContactForm = () => {
  ...
  return (
    <>
      ...
      <form className="w-full max-w-[806px] mx-auto bg-transparent border border-black rounded-lg font-poppins flex flex-col justify-center items-stretch gap-6 py-6 px-3 sm:p-6" onSubmit={handleSubmit}>
          ...
          <div className="flex gap-3 flex-wrap mt-4 sm:mt-2 -ml-2">
            {radio_choices.map((radio, index) => (
              <Radio
                ...
                icon= <RadioIcon /> onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined}              />
            ))}
          </div>
        </div>
        <div className="flex flex-col gap-2">
          ...
          <textarea
            title="message"
            placeholder="Message"
            className="bg-transparent border border-black rounded-sm text-base text-white placeholder:text-white py-3.5 px-4 focus:outline-0 focus:border-blue resize-none"
            rows={5}
            onChange={(e) => setMessage(e.target.value)}
          ></textarea>
        </div>
        <button type="submit" className="w-full bg-primary font-rubik font-poppins font-medium text-base py-3 rounded-lg text-center hover:bg-opacity-80 transition-all" disabled={loading}>
          {
